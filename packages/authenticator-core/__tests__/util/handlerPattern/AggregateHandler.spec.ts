...
describe('AggregateHandler', () => {
  ...
  function initMocks (configs: { canHandle: boolean, executeTime: number, toReturn: string }[]) {
    ...
    const aggregateMockHandler =
      new AggregateMockHandler(mockHandlerInfo.map((info) => info.mock()))
    return {
      mockHandlerInfo,

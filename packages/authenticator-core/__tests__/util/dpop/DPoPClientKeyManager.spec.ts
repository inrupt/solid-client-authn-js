...
describe('DPoPClientKeyManager', () => {
  ...
  describe('generateClientKeyIfNotAlready', () => {
    ...
    const hardCodedOIDCOptions: IOIDCOptions = OIDCHandlerCanHandleTests['legacyImplicitFlowOIDCHandler'][0].oidcOptions

    it('should generate a key and save it if one does not exist', async () => {
      const mocks = initMocks(null)
      await mocks.dPoPClientKeyManager.generateClientKeyIfNotAlready(hardCodedOIDCOptions)
      expect(mocks.StorageMockSetFunction)
        .toHaveBeenCalledWith('clientKey', JSON.stringify(mocks.JoseUtilityMockGenerateJWKResponse))
    })

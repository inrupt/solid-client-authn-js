...
describe('OIDCLoginHandler', () => {
  const defaultMocks = {
    ...
    issuerConfigFetcher: { fetchConfig: jest.fn(() => Promise.resolve({ arbitrary: 'config' })) },
    dPoPClientKeyManager: { generateClientKeyIfNotAlready: jest.fn(() => Promise.resolve()) },
  }
  function getInitialisedHandler(mocks: Partial<typeof defaultMocks> = defaultMocks) {
  ...
  it('should throw an error when called without an issuer', async () => {
    ...
    await expect(handler.handle({})).rejects.toThrowError('OIDCLoginHandler requires an oidcIssuer')
  })

  it('should indicate it when it can handle logins', async () => {
    const handler = getInitialisedHandler()

    await expect(handler.canHandle({ oidcIssuer: new URL('https://arbitrary.url') })).resolves.toBe(true)
  })

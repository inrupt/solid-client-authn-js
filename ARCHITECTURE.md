...
are tracked by a **Handler Aggregator**, which will have the first handler for which `canHandle`
...
app's control (its `redirect_uri`). The Issuer appends some query parameters to 
the IRI the Resource Owner is redirected to so that the Client can use to complete
the login flow. This is done when the developer calls `handleIncomingRedirect`,
